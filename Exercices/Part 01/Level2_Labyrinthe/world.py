...
class Hero(cocos.sprite.Sprite):
    def __init__(self):
        ...
        super(Hero, self).__init__(
            hero_image, (0, 0), anchor=(0, -TILE_SIZE[1] / 3))
...
class DungeonTileSet(cocos.tiles.TileSet):
    def __init__(self):
        ...
        glTexParameteri(ground_image.texture.target, GL_TEXTURE_MAG_FILTER,
                        GL_NEAREST)
        glTexParameteri(ground_image.texture.target, GL_TEXTURE_MIN_FILTER,
                        GL_NEAREST)

        obstacle_image = pyglet.image.load(OBSTACLE_IMAGE_PATH)
        glTexParameteri(obstacle_image.texture.target, GL_TEXTURE_MAG_FILTER,
                        GL_NEAREST)
        glTexParameteri(obstacle_image.texture.target, GL_TEXTURE_MIN_FILTER,
                        GL_NEAREST)

        goal_image = pyglet.image.load(GOAL_IMAGE_PATH)
        glTexParameteri(goal_image.texture.target, GL_TEXTURE_MAG_FILTER,
                        GL_NEAREST)
        glTexParameteri(goal_image.texture.target, GL_TEXTURE_MIN_FILTER,
                        GL_NEAREST)
...
class Game(cocos.scene.Scene):
    def __init__(self, dungeon_size):
        window = cocos.director.director.init(
            ...
            autoscale=True,
            caption='Level 2')
        window.set_size(SCREEN_SIZE[0] * SCALE_FACTOR,
    ...
    def draw(self, *args, **kwargs):
        ...
        super(Game, self).draw(*args, **kwargs)
    ...
    def set_view(self, screen_size):
        ...
        origin = (center[0] - int(dungeon_size[0] / 2 * TILE_SIZE[0]),
...
class Dungeon(cocos.tiles.RectMapLayer):
    def __init__(self, dungeon_size):
        ...
        super(Dungeon, self).__init__('dungeon', TILE_SIZE[0], TILE_SIZE[1],
                                      self.__cells)
    ...
    def set_view(self):
        ...
        super(Dungeon,
              self).set_view(0, 0, self.dungeon_size[0] * TILE_SIZE[0],
